...

{
    ...
    _button_nouv->setText(tr("New part"));
    _button_sauv->setText(tr("Save"));
    _button_charg->setText(tr("Load part"));
    _texte->setText(QString("%1%2").arg(tr("number of stroke : ")).arg(_nbcoup));
...
void MainWindow::aprops()
{
    ...
    infos.setIconPixmap(QPixmap(":/icon/annexe/240px-15-puzzle.png").scaled(50,50));
    infos.setText(tr("This application was created by Taise De Thèse NGANGA for the Qt project of L3 Informatique."));
    infos.setWindowTitle(tr("About Mystic Square "));
    infos.setWindowIcon(QIcon(":/icon/annexe/240px-15-puzzle.png"));
...
void MainWindow::move()
{
    ...
    if (hypot(row-row1,col-col1)==1) {
        ...
        _texte->setText(QString("%1%2").arg(_nbcoup>1?tr("number of strokes : "):tr("number of stroke : ")).arg(_nbcoup));
          _button_sauv->setDisabled(false);

        // vérifie si la partie est gagnée
        if (partiegagner()) {
            QMessageBox::information(this, "Fin!", QString("%1%2%3").arg(tr("congratulations, you won the Game in ")).arg(_nbcoup).arg(tr(" moves!")));
...
void MainWindow::nouvellepartie()
{
    ...
    if (_statutpartie == partie::encours) {
         QMessageBox msgBox(QMessageBox::Question,tr("Started a new game!"),tr("Abandon the current game for a new game?"),QMessageBox::Ok|QMessageBox::Cancel,this);
...
{
    charger_image(":/annexe/arbre-papillons.jpg");
    _dernierimg=":/annexe/arbre-papillons.jpg";
}

void MainWindow::charge_image_reseau()
{
    charger_image(":/annexe/reseau-informatique.jpeg");
    _dernierimg=":/annexe/reseau-informatique.jpeg";
}
...
{
    charger_image(":/annexe/foret.jpg");
    _dernierimg=":/annexe/foret.jpg";
}
...
{    int reply = 1;
     ...
     if (_statutpartie == partie::encours) {
         QMessageBox msgBox(QMessageBox::Question, tr("Quit a game!"), tr("Abandon the current game and quit application ?"),QMessageBox::Ok|QMessageBox::Cancel,this);
                QAbstractButton* okButton = msgBox.button(QMessageBox::Ok);
                QAbstractButton* cancelButton = msgBox.button(QMessageBox::Cancel);
                okButton->setText(tr("Yes"));
                cancelButton->setText(tr("No"));
...
void MainWindow::sauver_partie()
{
    if(_statutpartie==partie::terminer){
        QMessageBox::information(this,tr("Save part"),tr("please start or load the game to be able to save!"));
    }
    else{
   ...
   // Afficher une boîte de dialogue pour sélectionner le fichier de destination
     ...
     dialog.setLabelText(QFileDialog::Reject,tr("Cancel"));
      QString  filePath = dialog.getSaveFileName(this, tr("Save the part"), "../projet_diga/save", "*.save");
    if (!filePath.isNull()) {
        ...
        if (file.open(QIODevice::WriteOnly| QIODevice::Text)) {
            ...
            QString nomficher(tr("Congratulations, the game has been saved in the file: ")) ;
                 nomficher.append(info.fileName());
                 nomficher.append(" !");
            QMessageBox::information(this,tr("Game saved !"),nomficher);
        }

    }else {
            QMessageBox::warning(this, tr("Attention game not saved!"), tr("The game was not saved because you unsaved the game."));
        }
...
void MainWindow::charger_partie()
{
    ...
    if (_statutpartie == partie::encours) {
         QMessageBox msgBox(QMessageBox::Question,tr("Load a game!"), tr("if you load a game, you lose the current game, do you want to load a game?"),QMessageBox::Ok|QMessageBox::Cancel,this);
                QAbstractButton* okButton = msgBox.button(QMessageBox::Ok);
                QAbstractButton* cancelButton = msgBox.button(QMessageBox::Cancel);
                okButton->setText(tr("Yes"));
                cancelButton->setText(tr("No"));
    ...
    if ((reply == QMessageBox::Ok && _statutpartie == partie::encours) || _statutpartie == partie::terminer) {
        ...
        dialog.setLabelText(QFileDialog::Accept,tr("Save"));
        dialog.setLabelText(QFileDialog::Reject,tr("Cancel"));
         QString filePath = dialog.getOpenFileName(this, tr("Load the part"), "../projet_diga/save", "*.save");
        if (!filePath.isNull()) {
            QFile file(filePath);
...
if(verifie&&ens.size()==taille*taille){
    ...
    QMessageBox::warning(this, tr("unable to load game!"), tr("unable to load game, file corrupted"));
     }
...
void MainWindow::aproposqt()
{
    ...
    QString aboutQtText = QApplication::translate("QDialog", "<h3>About Qt</h3>"
        ...
        QMessageBox *dialog=new QMessageBox(QMessageBox::NoIcon,tr("About Qt"),"",QMessageBox::NoButton,this);
        dialog->setIconPixmap(QPixmap(":/icon/annexe/qtinfos.png").scaled(60,60));
